  ...
  return (
    <div className={cn('space-y-3', className)} role="status" aria-label="Loading">
      {Array.from({ length: count }, (_, i) => (
        <div
          key={i}
          className="animate-skeleton-pulse border-surface-300/20 bg-surface-100/80 rounded-2xl border p-5"
        >
          <div className="bg-surface-300/50 h-3 w-2/5 rounded-lg" />
          <div className="bg-surface-300/50 mt-3 h-3 w-4/5 rounded-lg" />
          <div className="bg-surface-300/50 mt-2 h-3 w-3/5 rounded-lg" />
        </div>
      ))}
    ...
    <div className={cn('grid gap-4', gridCols)} role="status" aria-label="Loading">
      {Array.from({ length: count }, (_, i) => (
        <div
          ...
          className="animate-skeleton-pulse border-surface-300/20 bg-surface-100/80 rounded-2xl border p-5"
        >
          <div className="bg-surface-300/50 h-3 w-1/3 rounded-lg" />
          <div className="bg-surface-300/50 mt-3 h-7 w-1/2 rounded-lg" />
          <div className="bg-surface-300/50 mt-2 h-3 w-2/3 rounded-lg" />
        </div>
      ))}
      <span className="sr-only">Loading...</span>
    </div>
...
export function TableSkeleton({ rows = 5, cols = 4 }: TableSkeletonProps) {
  return (
    <div
      className="border-surface-300/20 bg-surface-100/80 rounded-2xl border"
      role="status"
      aria-label="Loading"
    >
        ...
        <table className="w-full text-sm">
            ...
            <tr className="border-surface-300/20 border-b">
              {Array.from({ length: cols }, (_, i) => (
                <th key={i} className="px-4 py-3 text-left">
                  <div className="bg-surface-300/50 h-3 w-16 rounded-lg" />
                </th>
              ))}
            </tr>
          </thead>
          <tbody>
            {Array.from({ length: rows }, (_, i) => (
              <tr
                key={i}
                className="border-surface-300/10 border-b last:border-0"
              >
                {Array.from({ length: cols }, (_, j) => (
                  <td key={j} className="px-4 py-3">
                    <div
                      className={cn(
                        'bg-surface-300/50 h-3 rounded-lg',
...
export function EmptyState({
  icon = <Inbox className="text-surface-300 h-12 w-12" />,
  title = 'No data yet',
  message,
  action,
  className,
  ...
  if (!animated) {
    return (
      <div
        className={cn(
          'border-surface-300/30 bg-surface-50/50 flex flex-col items-center justify-center rounded-2xl border border-dashed',
          variants[variant],
          className,
        )}
      >
        <div className="text-surface-300 ">{icon}</div>
        <h3 className="text-surface-600 mt-4 text-base font-semibold">
          {title}
        </h3>
  ...
  const items = [
    ...
    <motion.div
      ...
      animate="visible"
    >
      <h3 className="text-surface-600 text-base font-semibold">{title}</h3>
    </motion.div>,
    ...(message
      ? [
          <motion.div
...
export function NotFoundState({
  ...
  return (
    <div className="flex flex-col items-center justify-center py-20">
      <FileQuestion className="text-surface-300 h-12 w-12" />
      <h2 className="text-surface-700 mt-4 text-xl font-semibold">{title}</h2>
