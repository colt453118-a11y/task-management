...
interface DependencyGraphClientProps {
  /** Server-rendered graph payload; null means the server load failed/not-found and the client should fetch. */
  initialGraph: DependencyGraphData | null;
  /** Server-rendered root task title (empty string when unavailable). */
  initialTaskTitle: string;
}

export function DependencyGraphClient({ initialGraph, initialTaskTitle }: DependencyGraphClientProps) {
  const params = useParams();
  const taskId = params.id as string;

  // When the server provided the graph, first paint already has real content.
  const [hadInitialData] = useState(() => initialGraph !== null);
  ...
  const [stats, setStats] = useState<GraphStats | null>(initialGraph?.stats ?? null);
  ...
  return (
    ...
    >
      ...
      {!loading && !error && stats && stats.totalNodes > 0 && (
        <motion.div
          initial={hadInitialData ? false : { opacity: 0, y: -8 }}
