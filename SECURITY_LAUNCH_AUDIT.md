...
| 12 | **P1 — High** | **Host-header SSRF in `serverFetchJson`.** The RSC data-seeding primitive built a credentialed (session-cookie-forwarding) fetch from the client-influenceable `Host` / `X-Forwarded-Host` header. | On a deploy not behind a Host-normalizing proxy (the bundled `docker-compose.prod`), a spoofed host exfiltrates the victim's session cookie → session theft / SSRF. Mitigated on Render by its proxy; still defense-in-depth. | **✅ Fixed** — calls the app's own API over loopback (`127.0.0.1:$PORT`), ignoring the Host header (PR #162); proven SSR seeding intact (LCP still ~200 ms) and a spoofed-`Host` request served from loopback. |
